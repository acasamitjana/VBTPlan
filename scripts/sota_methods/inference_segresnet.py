...
print('\n\n\n')
print('# --------------------------- #')
print('# VBT Segmentation: SegResNet #')
print('# --------------------------- #')
print('\n')
...
weightsfile = os.path.join(os.environ['PYTHONPATH'], 'models', 'model_densenet.pth')
...
if not os.path.exists(os.path.join(seg_dir, 'final_im.nii.gz')):
    ...
    if args.i.endswith('.nii') or args.i.endswith('.nii.gz'):
        ...
        subprocess.call(['ln', '-s', args.i, os.path.join(seg_dir, 'orig_im.nii.gz')])
        ...
        nib.save(im_proxy, os.path.join(seg_dir, 'orig_im.nii.gz'))

    data_dict = {'image': image.astype('float32'), 'image_v2r': orig_vox2ras, 'orig_image': copy.deepcopy(image), 'orig_v2r': copy.deepcopy(orig_vox2ras)}

    if args.debug:
        img = nib.Nifti1Image(data_dict['image'], data_dict['image_v2r'])
    ...
    tensor_dict = testing_session.step_full(data_dict, {'seg': model_dict['full']})
